...
class SCTypeHandler:
	...
	def coeff(self, att_type, def_type1, def_type2 = None):
		...
		if type(def_type1) == list:
			if len(def_type1) == 1:
				return self.types_matrix[att_type][def_type1[0]]
			else:
				return self.types_matrix[att_type][def_type1[0]] * self.types_matrix[att_type][def_type1[1]]
	...
	def findWeaknesses(self, pokemon):
		...
		for a_type in SCTypeHandler.types:
			c = self.coeff(a_type, type1, type2)
			if c >= 2.0:
				weak_types.append(a_type)
...
if __name__ == "__main__":
	...
	for def_type1 in SCTypeHandler.types:
		for def_type2 in SCTypeHandler.types:
			num_weaknesses = 0
			if def_type1 == def_type2:
				def_type2 = None
			for a_type in SCTypeHandler.types:
				c = TYPE_HANDLER.coeff(a_type, def_type1, def_type2)
				if c >= 2.0:
					num_weaknesses += 1
			if num_weaknesses not in res:
				res[num_weaknesses] = []
			res[num_weaknesses].append([def_type1, def_type2])
	
	for i in range(18):
		print("Num weaknesses = {}".format(i))
		if i in res:
			print(res[i])
